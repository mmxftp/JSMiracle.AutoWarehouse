...
{
    public class IMS_UP_RoMo_Dal : DataLayer<IMS_UP_RoMo>, IPermission
    {
        public IList<TreeModel> GetPermissionInfo(string roleid)
        {
            // 根节点
            var rootQueryable =
                this.DbContext.IMS_UP_ModuT.Where(n => n.ParentID == -1).OrderBy(n => n.SortID);
            ...
            var perList = this.DbContext.IMS_UP_RoMoT.Where(n => n.RoleId == roleid);
            ...
            var moduleQueryable = from m in this.DbContext.IMS_UP_ModuT
            ...
            var funQueryable = from f in this.DbContext.IMS_UP_MoFnT
                               join p in perList
        ...
        private void FillFunctionList(string roleid, int moduleid, ref List<IMS_UP_RoMo> permissionAllQueryable)
        {
            if (permissionAllQueryable == null)
                permissionAllQueryable = new List<IMS_UP_RoMo>();

            permissionAllQueryable.Add(new IMS_UP_RoMo()
            {
                ModuleID = moduleid,
                RoleId = roleid,
                FunctionID = -1
            ...
            var functionQuery = from f in this.DbContext.IMS_UP_MoFnT
        ...
        {
            var role = this.DbContext.IMS_UP_RoleT.Where(n => n.RoleID == roleid);
            ...
            // 当前角色的所有权限
            var permissionExistsQueryable = from p in this.DbContext.IMS_UP_RoMoT
            ...
            // 所有需处理的权限
            List<IMS_UP_RoMo> permissionAllQueryable = new List<IMS_UP_RoMo>();

            // 根节点
            if (parentid == -1 && moduleid == -1 && functionid == -1)
            {
                var rootModuleQuery = this.DbContext.IMS_UP_ModuT.Where(n => n.ParentID == -1);

                foreach (var r in rootModuleQuery)
                {
                    var moduleQuery = from n in this.DbContext.IMS_UP_ModuT
            ...
            if (check)
            {
                foreach (var n in permissionAllQueryable)
                {
                    ...
                    if (!permissionExistsQueryable.Any(p => p.ModuleID == n.ModuleID && p.FunctionID == n.FunctionID))
                    {
                        ...
                        this.DbContext.IMS_UP_RoMoT.Add(n);
                        effectRowCount++;
                    }
                }
            ...
            else
            {
                ...
                var removePermissionList = from a in permissionAllQueryable
                                           join p in this.DbContext.IMS_UP_RoMoT
                                           on new
                                           {
                                               a.RoleId,
                                               a.ModuleID,
                ...
                this.DbContext.IMS_UP_RoMoT.RemoveRange(removePermissionList);
                // 提交数据库
                this.DbContext.SaveChanges();
            }
        ...
        public PermissionViewModule GetPermissionListByUserID(string userid)
        {
            ...
            var modList = from m in this.DbContext.IMS_UP_ModuT
                          join p in this.DbContext.IMS_UP_RoMoT.Where(n => n.FunctionID == -1)
                          on m.ModuleID equals p.ModuleID into v_per
                          from v in v_per.DefaultIfEmpty()
                          join r in this.DbContext.IMS_UP_RoUrT
            ...
            var funList = from f in this.DbContext.IMS_UP_MoFnT
                          join p in this.DbContext.IMS_UP_RoMoT
                          on new
                            {
                                mod = f.ModuleID,
        ...
        public PermissionViewModule GetAllPermission()
        {
            var permissions = new PermissionViewModule(
                this.DbContext.IMS_UP_ModuT.ToList()
                , this.DbContext.IMS_UP_MoFnT.ToList());

            return permissions;
        }


        public List<IMS_UP_RoMo> GetPermissionListByRoleID(string roleid)
        {
            if (string.IsNullOrEmpty(roleid))
                throw new JsMiracleException("roleid 不可为空");

            var data =this.DbContext.IMS_UP_RoMoT.Where(n => n.RoleId.Equals(roleid, StringComparison.CurrentCultureIgnoreCase));
            return data.ToList();
        }
    }
}
